...
observed biomass, the atmospheric CO2 concentration to use for each year, and
the weight to assign to each year:
...
is now `r optim_res[['value']]`, which is
`r round(100 * abs(optim_res[['value']] - default_error) / optim_res[['value']], 1)`
percent
`r if (optim_res[['value']] > default_error) {'larger'} else {'smaller'}`
than with the default parameter values.
